...
echo 'EXCLUDED_ARCHS=$(inherited) arm64 $(EXCLUDED_ARCHS__EFFECTIVE_PLATFORM_SUFFIX_$(EFFECTIVE_PLATFORM_SUFFIX)__NATIVE_ARCH_64_BIT_$(NATIVE_ARCH_64_BIT))' >> /tmp/tmp.xcconfig
...
carthage bootstrap $CARTHAGE_VERBOSE --platform ios --color auto --no-cache-builds --no-use-binaries
